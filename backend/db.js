...
// Re-uploading the same mobile+docket row updates it instead of duplicating.
// Falls back to store code + row number when mobile and docket are both absent.
async function upsertDocket(data, rowKey) {
  ...
  const key = docketNo || (mobile ? ('MOB-' + mobile) : (storeCode ? ('STORE-' + storeCode.toLowerCase() + (rowKey ? '-' + rowKey : '')) : ''));
  if (!mobile && !docketNo && !storeCode) return null;
  const values = {
    mobile,
    docket_no: key,
